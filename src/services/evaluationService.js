 ...
 *
 * Yerel Ollama LLM kullanıldığı için rate-limit bekleme süreleri kaldırıldı.
 */
...
/**
 * Ollama LLM'yi hakem olarak kullanarak Context Relevance, Faithfulness ve
...
export function exportToCSV(results) {
    ...
    const rows = results.map(r => [
        r.method, r.questionId,
        esc(r.question), esc(r.answer), esc(r.groundTruth),
        r.contextRelevance, r.faithfulness, r.answerRelevance,
        r.bleu ?? 0, r.rouge1 ?? 0, r.rouge2 ?? 0, r.rougeL, r.meteor ?? 0,
        r.precisionAtK ?? 0, r.recallAtK ?? 0, r.mrr ?? 0, r.ndcg ?? 0,
        r.latency, r.usedTokens ?? r.estimatedTokens ?? 0,
        r.error ? 'EVET' : 'HAYIR'
    ].join(','))
